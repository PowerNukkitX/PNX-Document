...
function switchTheme(noNotice) {
    ...
    const prismThemeEle = document.getElementById("prism-theme");
    if (clazz.contains("mdui-theme-layout-dark")) {
        ...
        prismThemeEle.href = prismThemeEle.href.replace("dark", "light");
        if (!noNotice) {
        ...
        prismThemeEle.href = prismThemeEle.href.replace("light", "dark");
        if (!noNotice) {
...
function createCssEle(id, link) {
    const cssEle = document.createElement("link");
    cssEle.setAttribute("rel", "stylesheet");
    cssEle.setAttribute("type", "text/css");
    cssEle.setAttribute("href", link);
...
function checkLanguage() {
    ...
    for (const each in window.languageLinks) {
        if (!window.languageLinks[each]) {
            for (const ele of document.querySelectorAll(`[multi-lang="${each}"]`)) {
                ele.parentElement.classList.add("mdui-hidden");
            }
        } else if (each !== currentLanguageId) {
            for (const ele of document.querySelectorAll(`[multi-lang="${each}"]`)) {
                ele.setAttribute("href", window.languageLinks[each]);
            }
        }
    }
}
...
function switchLanguage(ele) {
    const languageId = ele.getAttribute("lang");
    window.localStorage.setItem("userLanguage", languageId);
...
function sideBarCollapse() {
    for (const ele of document.getElementsByClassName("category-item")) {
...
function sideBarTryOpen() {
    if (window.visualViewport.width >= 1024) {
        (new mdui.Drawer("#catalogue-drawer")).open();
    }
}
